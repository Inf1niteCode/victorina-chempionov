...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Показываем generic сообщение даже при ошибке
      setSent(true);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--bg-deep)' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
          ...
          <AnimatePresence mode="wait">
            {sent ? (
              <motion.div key="sent" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <h3 className="font-bold text-lg" style={{ color: 'var(--text-primary)' }}>Письмо отправлено</h3>
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                  Если этот email зарегистрирован, вы получите письмо со ссылкой для сброса пароля.
                  Ссылка действительна <strong style={{ color: 'var(--text-primary)' }}>1 час</strong>.
                </p>
        ...
        <p className="text-center mt-6 text-sm" style={{ color: 'var(--text-muted)' }}>
          <Link to="/auth" className="hover:underline" style={{ color: 'var(--accent-blue)' }}>
            ← Вернуться к входу
          </Link>
        </p>
      </motion.div>
